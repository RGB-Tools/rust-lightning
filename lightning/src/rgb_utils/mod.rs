...
use rgb_lib::{
    ...
    wallet::{rust_only::ColoringInfo, AssetIface, DatabaseType, Outpoint, WalletData},
...
pub(crate) fn color_commitment<SP: Deref>(
    channel_context: &ChannelContext<SP>,
    commitment_transaction: &mut CommitmentTransaction,
    counterparty: bool,
) -> Result<(), ChannelError>
where
    <SP as std::ops::Deref>::Target: SignerProvider,
{
    ...
    let funding_outpoint = channel_context
    ...
    .unwrap() as u32;
    let vout_p2wsh = commitment_transaction
        .trust()
    ...
    commitment_transaction.built = BuiltCommitmentTransaction {
...
fn _append_pending_extension(path: &PathBuf) -> PathBuf {
...
pub(crate) fn handle_funding(
    ...
    let consignment_path =
        ldk_data_dir.join(format!("consignment_{}", temporary_channel_id.to_hex()));
    consignment
        .save_file(&consignment_path)
        .expect("unable to write file");
...
pub(crate) fn filter_first_hops(
    ldk_data_dir: &Path,
    payment_hash: &PaymentHash,
    first_hops: &mut Vec<&ChannelDetails>,
) -> ContractId {
